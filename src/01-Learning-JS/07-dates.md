...
<!-- E1 -->
```js
// E1 — parse "MM/DD/YYYY" to Date objects
const parseMDY = utcParse("%m/%d/%Y")

let ballotsWithDateObjs = nc2024SampleVoters.map(voter => {
  return {
    ...voter,
    ballot_req_dt_obj: parseMDY(voter.ballot_req_dt)
  }
}
)
```
...
<!-- E2 -->
```js
// E2 — format to "Wed., January 27, 1981"
const fmtAbbrev = utcFormat("%a")        
const fmtRest   = utcFormat("%B %d, %Y") 

let updatedBallots = ballotsWithDateObjs.map(voter => {
  const d = voter.ballot_req_dt_obj
  // Add a dot after the weekday abbreviation:
  const pretty = `${fmtAbbrev(d)}., ${fmtRest(d)}`
  return { ...voter, ballot_req_dt_pretty: pretty }
}
)
```
